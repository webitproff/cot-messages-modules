...
$t1 = new XTemplate(cot_tplfile(array('messages', 'dashboard')));
...
if ($act == "new") {
    if (!empty($to)) {
        if ($to != $usr['id']) {
            ...
            cot_redirect(html_entity_decode(cot_url('dashboard', 'm=messages')));
        }
    }
...
$catpatharray[] = array(cot_url('dashboard', 'm=messages'), $L['msg_dashboard']);
$catpath = cot_breadcrumbs_uk($catpatharray, $cfg['homebreadcrumb'], true);
#cot_breadcrumbs_uk($title, $cfg['homebreadcrumb'])
$t->assign(array(
    'BREADCRUMBS' => $catpath,
));
...
foreach ($dialogs as $row) {
    ...
    $t1->assign(cot_generate_usertags($_opponent, 'MSG_OPPONENT_'));
    $t1->assign(cot_generate_usertags($from_user, 'MSG_FROM_USER_'));
    ...
    $t1->assign(array(
        ...
        'MSG_DIAOG_URL' => cot_url('dashboard', 'm=messages&chat=' . $row['id']),
...
// -------------------- END LIST
$t1->assign(array(
    'MSG_LIST' => $t1->text('MSG_LIST'),
    'MSG_ROW' => $t1->text('MSG_ROW'),
));
