...
$sql = $db->query("SELECT * FROM $db_config
	WHERE config_cat='messages' AND config_name='hideset' LIMIT 1")->fetch();

if (((int)$sql['config_value'] +$cfg['messages']['timesend']*60) < (int)$sys['now']) 
{
 $reg_upd['config_value'] = $sys['now'];                                          
 $db->update($db_config, $reg_upd, "config_cat='messages' AND config_name='hideset'");  

 $sql = $db->query("SELECT COUNT(id) AS messagescount, max(m.id) AS new_last, u.* FROM $db_messages_msg AS m
 ...
 { 
   ...
   {
	  $tags = $send_urr;
    $rbody = cot_rc($L['msg_mail_first'], $tags).cot_declension($send_urr['messagescount'], $Ls['Privatemessages']).'.';   		
	  cot_mail($send_urr['user_email'], $L['msg_mail_header'], $rbody);
